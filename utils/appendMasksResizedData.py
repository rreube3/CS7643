...
path_to_patches: str = "A:/DATA_4D_Patches/Resized_Datasets/Resized_Datasets/"
...
if __name__ == "__main__":
    appendMasks("CHASE_DB1/", "labels/", lambda p, f: p + "mask/mask" + f.replace("png", "jpg"))
    appendMasks("DRIVE/", "1st_manual/", drive_mask_path_fn, use_subdirs=True)
    appendMasks("Stare/", "labels/", lambda p, f: p + "mask/mask" + f)

    path: str = "A:/DATA_4D_Patches/Resized_Datasets/Resized_Datasets/Stare/"
    files = set()
    ...
    for filename in os.listdir(f"{path}images/"):
        if filename not in files:
            to_be_removed.append(f"{path}images/{filename}")
    for fi in to_be_removed:
        os.remove(fi)

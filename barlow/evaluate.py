...
# removed imagenet specific reference
...
def main():
    ...
    # removed imagenet subnet reference
...
def main_worker(gpu, args):
    ...
    # TODO: init unet model here
    model = models.resnet50().cuda(gpu)
    state_dict = torch.load(args.pretrained, map_location='cpu')
    missing_keys, unexpected_keys = model.load_state_dict(state_dict, strict=False)
    # TODO: determine appropriate missing keys
    assert missing_keys == ['fc.weight', 'fc.bias'] and unexpected_keys == []
    # TODO: init decoder weights from encoder?
    # delete references to fc layers
    # model.fc.weight.data.normal_(mean=0.0, std=0.01)
    # model.fc.bias.data.zero_()
    if args.weights == 'freeze':
        # model.requires_grad_(False)
        # model.fc.requires_grad_(True)
        # TODO: freeze anything only for eval of full model?
    ...
    # TODO: determine loss function of unet
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.weights == 'finetune':
            ...
            print('You must be running this in either finetune or freeze mode')
            assert False
